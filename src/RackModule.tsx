...
<div id="stock-calc-modal">
  ...
  <div class="calc-grid">
    ...
    loadScript("https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js").then(async () => {
      ...
      let rackPalettesMap: Record<string, number[]> = {}; // article (lowercase, résolu) -> liste des quantités, une par palette, vues aux niveaux 2 et 3
      ...
      // Chaque palette du rack devient sa PROPRE case de comptage (pas une somme globale).
      const computeRackPalettes = (allWalls: any): Record<string, number[]> => {
        ...
        Object.values(allWalls || {}).forEach((wallPositions: any) => {
          Object.entries(wallPositions || {}).forEach(([key, data]: any) => {
            ...
            if ((niveau === 2 || niveau === 3) && data?.produit) {
              ...
              if (qty > 0) { if (!map[nom]) map[nom] = []; map[nom].push(qty); }
            }
          });
        });
        return map;
      ...
      (window as any).sStartSession = (team: string) => {
        ...
        loadComptages(team).then(async () => {
          ...
          // Auto-comptage : une case par palette vue en rack (niveaux 2/3), pas encore comptée manuellement
          articles.forEach(a => {
            if (a.compte1 === null || a.compte1 === undefined) {
              ...
              if (palettes && palettes.length) {
                ...
                slots.forEach((qty, i) => { a["compte" + (i + 1)] = qty; });
      ...
      const sRenderTable = () => {
        ...
        rows.forEach(a => {
          ...
          let inp = `<input class="qty-in" data-loc="1" type="number" min="0" inputmode="decimal" value="${q1}"${a._autoRackSlots?.[0] ? ` style="${autoStyle}" title="📦 Vu en rack N2/N3"` : ""} oninput="sSetCount(${a.id},1,this.value)" onchange="sSetCount(${a.id},1,this.value)">`;
          let lastFilled = 1;
          locs.forEach((v: any, i: number) => { if (i > 0 && v !== null && v !== undefined) { inp += `<input class="qty-in" data-loc="${i + 1}" type="number" min="0" inputmode="decimal" value="${v > 0 ? v : ""}"${a._autoRackSlots?.[i] ? ` style="${autoStyle}" title="📦 Vu en rack N2/N3"` : ""} oninput="sSetCount(${a.id},${i + 1},this.value)" onchange="sSetCount(${a.id},${i + 1},this.value)">`; lastFilled = i + 1; } });
          ...
          html += `<tr data-id="${a.id}">
            <td style="font-weight:500">${artLabel}${a.comment ? `<br><span style="font-size:11px;color:#6b7280;font-style:italic">${a.comment}</span>` : ""}${lotsStr ? `<br><span style="font-size:10px;color:#9ca3af">${lotsStr}</span>` : ""}${a._autoRackSlots?.some((x: boolean) => x) ? `<br><span class="s-auto-rack-badge" style="font-size:10px;color:#8b5cf6;font-weight:700">📦 ${a._autoRackSlots.filter((x: boolean) => x).length} palette${a._autoRackSlots.filter((x: boolean) => x).length > 1 ? "s" : ""} vue${a._autoRackSlots.filter((x: boolean) => x).length > 1 ? "s" : ""} en rack N2/N3 (cases violettes)</span>` : ""}<br>${moveBtn}</td>
