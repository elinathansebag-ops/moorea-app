...
export function RackModule({ onClose }: { onClose: () => void }) {
  ...
  const [allWallsPositions, setAllWallsPositions] = useState<Record<string, Record<string, PalettePos>>>({});
  ...
  // ─── FIREBASE: TOUTES les positions, tous murs (pour retrouver les vrais noms déjà posés) ───
  useEffect(() => {
    const u = onValue(ref(db, "rack_positions"), snap => {
      setAllWallsPositions(snap.val() || {});
    });
    return () => u();
  }, []);
  ...
  // ─── LISTE DE TOUS LES NOMS UTILISÉS EN RACK (modèles + palettes réellement posées, tous murs) ───
  // Important : une palette posée AVANT un renommage de modèle garde son ancien nom "en dur" —
  // on doit donc aussi lister les noms réels trouvés dans les palettes, pas que les modèles actuels.
  const placedProductNames = Object.values(allWallsPositions || {}).flatMap((wallPositions: any) =>
    Object.values(wallPositions || {}).map((p: any) => p?.produit).filter(Boolean)
  );
  const allRackNames = [...new Set([
    ...WALL_IDS.flatMap(w => getPresetsForWall(w).map(p => p.produit)),
    ...placedProductNames,
