...
<div id="stock-calc-modal">
  ...
  <div class="calc-grid">
    ...
    (async () => {
      ...
      // (avant elles étaient locales et se réinitialisaient, d'où le bug "marche puis plus rien")
      (window as any)._calcExpr = (window as any)._calcExpr || "";
      (window as any)._calcCurrent = (window as any)._calcCurrent || "0";
      (window as any)._calcJustEvaled = (window as any)._calcJustEvaled ?? false;
      let calcExpr = (window as any)._calcExpr, calcCurrent = (window as any)._calcCurrent, calcJustEvaled = (window as any)._calcJustEvaled;
      ...
      (window as any).sAddArticleManuel = async () => {
        ...
        // Sauvegarder l'article en Firestore pour qu'il persiste dans le PDF généré (generateStockPDF relit depuis Firestore)
        if (currentImportId) {
          try {
            const snap = await getDoc(doc(db, "stocks", currentImportId));
            if (snap.exists()) {
              ...
              // Ajouter le nouvel article au format Firestore (sans _extra et compte qui sont temp)
              existingArts.push({ id: newArt.id, equipe: newArt.equipe, famille: newArt.famille, code: newArt.code, article: newArt.article, nb_colis: newArt.nb_colis, lot: newArt.lot, lots: newArt.lots, lotsQty: newArt.lotsQty });
              await setDoc(doc(db, "stocks", currentImportId), { ...data, articles: existingArts });
            }
          } catch (e) { console.warn("Erreur sauvegarde article manuel en Firestore:", e); }
        }
      ...
      const sRenderConfig = () => {
        ...
        tbody.innerHTML = rows.map(a => {
          ...
          const ifcoHtml = fusionMode ? "" : `<label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:11.5px;color:${isIfco ? "#1a6b3a" : "#9ca3af"};font-weight:${isIfco ? 700 : 500}" onclick="event.stopPropagation()"><span class="mrq-case-conteneur" style="--case-couleur:#1a6b3a"><input type="checkbox" class="mrq-case-native" data-enc="${enc}" ${isIfco ? "checked" : ""} onchange="sToggleIfco(this.dataset.enc,this.checked)"/><span class="mrq-case-visuelle"></span></span>Caisse IFCO</label>`;
      ...
      (window as any).sCalcNum = (n: string) => {
        if ((window as any)._calcJustEvaled) { (window as any)._calcCurrent = ""; (window as any)._calcJustEvaled = false; }
        if (n === "." && (window as any)._calcCurrent.includes(".")) return;
        (window as any)._calcCurrent = (window as any)._calcCurrent === "0" && n !== "." ? n : (window as any)._calcCurrent + n;
        const r = document.getElementById("s-calc-result"); if (r) r.textContent = (window as any)._calcCurrent;
      };
      (window as any).sCalcOp = (op: string) => {
        (window as any)._calcJustEvaled = false;
        if (op === "±") { (window as any)._calcCurrent = String(parseFloat((window as any)._calcCurrent) * -1); const r = document.getElementById("s-calc-result"); if (r) r.textContent = (window as any)._calcCurrent; return; }
        if (op === "%") { (window as any)._calcCurrent = String(parseFloat((window as any)._calcCurrent) / 100); const r = document.getElementById("s-calc-result"); if (r) r.textContent = (window as any)._calcCurrent; return; }
        (window as any)._calcExpr += (window as any)._calcCurrent + " " + op + " ";
        const e = document.getElementById("s-calc-expr"); if (e) e.textContent = (window as any)._calcExpr;
        (window as any)._calcCurrent = "0";
      };
      (window as any).sCalcEqual = () => {
        try {
          const full = (window as any)._calcExpr + (window as any)._calcCurrent;
          ...
          (window as any)._calcCurrent = String(r); (window as any)._calcExpr = ""; (window as any)._calcJustEvaled = true;
        } catch { const rd = document.getElementById("s-calc-result"); if (rd) rd.textContent = "Erreur"; (window as any)._calcCurrent = "0"; (window as any)._calcExpr = ""; }
      };
      (window as any).sCalcClear = () => {
        (window as any)._calcCurrent = "0"; (window as any)._calcExpr = ""; (window as any)._calcJustEvaled = false;
      ...
      (window as any).sCalcBackspace = () => {
        if ((window as any)._calcJustEvaled) { (window as any)._calcCurrent = "0"; (window as any)._calcJustEvaled = false; }
        else if ((window as any)._calcCurrent.length > 1) { (window as any)._calcCurrent = (window as any)._calcCurrent.slice(0, -1); }
        else { (window as any)._calcCurrent = "0"; }
        const r = document.getElementById("s-calc-result"); if (r) r.textContent = (window as any)._calcCurrent;
      };
      (window as any).sCalcUse = () => {
        ...
        if (val && calcLastFocused && document.body.contains(calcLastFocused as HTMLElement)) {
          ...
          (calcLastFocused as HTMLInputElement).dispatchEvent(new Event("input", { bubbles: true }));
        } else if (val) {
          console.warn("Calculette : aucun champ actif trouvé. Valeur : " + val);
        }
        const modal = document.getElementById("stock-calc-modal");
        if (modal) { modal.classList.remove("open"); modal.style.display = "none"; }
      };
      // Si document.addEventListener n'a pas encore été appelé, on le fait ici pour la calculette
      if (!document.body.dataset.calcListenerAdded) {
        document.body.dataset.calcListenerAdded = "1";
