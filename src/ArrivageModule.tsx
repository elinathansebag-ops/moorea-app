...
// ─── MESURES RÉPÉTÉES SUR UN ARRIVAGE (poids de barquettes, températures) ───
// Certains produits — les haricots verts en particulier — demandent plusieurs pesées de
// barquettes et plusieurs relevés de température sur un même arrivage. On stocke donc des
// LISTES horodatées plutôt qu'une valeur unique, pour pouvoir reconstituer la chronologie
// des contrôles en cas de litige fournisseur.
export type Mesure = { valeur: string; at: number };

// Relit une liste venant de Firebase en tolérant les arrivages anciens (champ absent,
// valeurs numériques, horodatage manquant).
export function lireMesures(v: any): Mesure[] {
  // Firebase rend un tableau si les clés sont 0..n contiguës, mais un OBJET indexé dès
  // qu'il y a un trou — on accepte donc les deux formes pour ne jamais perdre un relevé.
  const brut: any[] = Array.isArray(v)
    ? v
    : (v && typeof v === "object" ? Object.keys(v).sort((a, b) => Number(a) - Number(b)).map(k => v[k]) : []);
  return brut
    .filter((m: any) => m != null && String(m?.valeur ?? "").trim() !== "")
    .map((m: any) => ({ valeur: String(m.valeur).trim(), at: Number(m?.at) || 0 }));
}

// Ne persiste que les lignes réellement renseignées : une ligne ajoutée puis laissée vide
// ne doit pas polluer l'historique.
function mesuresAEnregistrer(liste: Mesure[]) {
  return liste
    .filter(m => String(m.valeur).trim() !== "")
    .map(m => ({ valeur: String(m.valeur).trim(), at: m.at || Date.now() }));
}

export function formaterHeureMesure(at: number) {
  if (!at) return "";
...
export function ProduitRow({ arrivage, onValidate, onDelete, onOuvreRapport, onReporterDate, selectMode, selected, onToggleSelect, gencodeArticles }: { arrivage: any; onValidate: any; onDelete: any; onOuvreRapport: any; onReporterDate?: (arrivage: any, nouvelleDateFr: string) => void; selectMode?: boolean; selected?: boolean; onToggleSelect?: (id: string) => void; gencodeArticles?: any[] }) {
  ...
  const modifierMesure = (setter: React.Dispatch<React.SetStateAction<Mesure[]>>, idx: number, val: string) =>
    setter(prev => prev.map((m, i) => i === idx ? { ...m, valeur: val } : m));
  const retirerMesure = (setter: React.Dispatch<React.SetStateAction<Mesure[]>>, idx: number) =>
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      update(ref(db, `arrivages/${arrivage.id}`), {
        ...
        poids_barquettes: mesuresAEnregistrer(poidsBarquettes),
        temperatures: mesuresAEnregistrer(temperatures),
      }).catch(() => {});
    }, 800);
    return () => clearTimeout(t);
  }, [dlc, tracaList, poidsBarquettes, temperatures, arrivage.id]);
        ...
        {!isSimple && [
          ...
          <div key={sec.cle} style={{ flex: 1, minWidth: 210, background: "#f9fafb", border: "1.5px solid #e5e7eb", borderRadius: 10, padding: "8px 12px" }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: "#6b7280" }}>
              {sec.titre}{sec.liste.length > 0 ? ` (${sec.liste.length})` : ""}
            </span>
            <div style={{ display: "flex", flexDirection: "column", gap: 5, marginTop: 5 }}>
              {sec.liste.map((m, idx) => (
                <div key={idx} style={{ display: "flex", gap: 5, alignItems: "center" }}>
                  <input type="number" inputMode="decimal" step={sec.step} min={sec.min}
                    value={m.valeur}
                    placeholder="0"
                    onChange={e => modifierMesure(sec.setter, idx, e.target.value)}
                    style={{ flex: 1, minWidth: 0, padding: "4px 6px", border: "1.5px solid #e5e7eb", borderRadius: 7, fontSize: 12, fontWeight: 700, outline: "none", color: "#1a2e1a" }}
                  />
                  <span style={{ flexShrink: 0, fontSize: 10, color: "#9ca3af", fontWeight: 600, whiteSpace: "nowrap" }}>
                    {formaterHeureMesure(m.at)}
                  </span>
                  <button onClick={() => retirerMesure(sec.setter, idx)} title="Retirer cette mesure"
              ...
              <button onClick={() => ajouterMesure(sec.setter)}
...
export function DateBlock({ date, arrivages, arrivagesArchives, onValidate, onDelete, onOuvreRapport, onImprimerMulti, onReporterDate, selectMode, selectedArrivages, onToggleSelect, onScan, gencodeArticles }: any) {
  ...
  // ─── RÉCAP WHATSAPP DU JOUR ───
  // Reprend les statuts des arrivages de CE jour uniquement (en attente + traités), là où le
  // bouton de la barre du haut récapitule toute la liste sans distinction de date.
  const recapWhatsAppJour = () => {
    const tous = [...arrivages, ...(arrivagesArchives || [])];
    if (!tous.length) { alert(`Aucun arrivage pour le ${date}.`); return; }
    ...
    const lignes: string[] = [];
    Object.keys(parFourn).sort().forEach(fourn => {
      const arts = parFourn[fourn];
      const refus = arts.filter((a: any) => a.statut === "refusé");
      const reserves = arts.filter((a: any) => a.statut === "sous réserve");
      const detail = (a: any) => `Lot ${a.lot_interne || "-"} · ${a.produit || ""} · ${a.quantite || "-"} colis`;

      if (refus.length) refus.forEach((a: any) => lignes.push(`❌ ${fourn} - Refus · ${detail(a)}`));
      if (reserves.length) reserves.forEach((a: any) => lignes.push(`⚠️ ${fourn} - Réserve · ${detail(a)}`));
      if (refus.length || reserves.length) return;

      if (arts.every((a: any) => a.statut === "validé")) lignes.push(`✅ ${fourn}`);
      else if (arts.every((a: any) => a.statut === "en attente")) lignes.push(`📦 ${fourn} - Pas encore reçu`);
      else lignes.push(`⏳ ${fourn} - En cours`);
    });

    // Ligne de synthèse : l'état du jour d'un coup d'œil, sans avoir à compter les lignes.
    const n = (s: string) => tous.filter((a: any) => a.statut === s).length;
    const synthese = [
      `${tous.length} article${tous.length > 1 ? "s" : ""}`,
  ...
  const telechargerTracabilite = () => {
    ...
    const rows = tous.map((a: any, i: number) => {
      // Affiche UNIQUEMENT les poids du rapport (contrôle aléatoire), pas les poids de base d'arrivage
      const poidsBrut = a.rapport?.poids_brut || "-";
      const poidsNet = a.rapport?.poids_net || "-";
      const controle = (poidsBrut !== "-" || poidsNet !== "-")
        ? `⚖️ Brut: ${esc(poidsBrut)} kg | 🥬 Net: ${esc(poidsNet)} kg`
        : "-";
      return `
...
<body>
  ...
  <table>
    <thead><tr><th>Fournisseur</th><th>Article</th><th>Lot Moorea</th><th>N° traçabilité</th><th>Qté</th><th>Contrôle poids</th></tr></thead>
    <tbody>${rows}</tbody>
  </table>
...
// ═══════════════════════════════════════════════════════════════════════════
// HISTORIQUE DES MESURES — poids de barquettes et températures relevés sur les arrivages.
// Traverse TOUS les arrivages (y compris archivés, puisque la validation ne fait que poser
// un drapeau `archived` sans supprimer la ligne), pour permettre un suivi sur plusieurs
// semaines : c'est l'intérêt même d'un historique de température.
// ═══════════════════════════════════════════════════════════════════════════
export function HistoriqueMesures({ arrivages, onClose }: { arrivages: any[]; onClose: () => void }) {
  const [q, setQ] = useState("");
  const [du, setDu] = useState("");
  const [au, setAu] = useState("");
  const [type, setType] = useState<"poids" | "temperature">("poids");

  const lignes = useMemo(() => {
    const out: { type: "poids" | "temperature"; valeur: string; at: number; a: any }[] = [];
    (arrivages || []).forEach((a: any) => {
      lireMesures(a?.poids_barquettes).forEach(m => out.push({ type: "poids", valeur: m.valeur, at: m.at, a }));
      lireMesures(a?.temperatures).forEach(m => out.push({ type: "temperature", valeur: m.valeur, at: m.at, a }));
    });
    return out.sort((x, y) => y.at - x.at);
  }, [arrivages]);

  const filtrees = useMemo(() => {
    const bDu = du ? new Date(`${du}T00:00:00`).getTime() : null;
    const bAu = au ? new Date(`${au}T23:59:59`).getTime() : null;
    const terme = q.trim().toLowerCase();
    return lignes.filter(l => {
      if (l.type !== type) return false;
      if (terme) {
        const hay = `${l.a?.produit || ""} ${l.a?.fournisseur || ""} ${l.a?.lot_interne || ""} ${l.a?.lot_fournisseur || ""}`.toLowerCase();
        if (!hay.includes(terme)) return false;
      }
      if (bDu !== null && l.at < bDu) return false;
      if (bAu !== null && l.at > bAu) return false;
      return true;
    });
  }, [lignes, q, du, au, type]);

  const stats = useMemo(() => {
    const nums = filtrees.map(l => parseFloat(String(l.valeur).replace(",", "."))).filter(n => !isNaN(n));
    if (!nums.length) return null;
    return {
      n: nums.length,
      moy: nums.reduce((s, n) => s + n, 0) / nums.length,
      min: Math.min(...nums),
      max: Math.max(...nums),
      stddev: Math.sqrt(nums.reduce((s, n) => s + Math.pow(n - (nums.reduce((a, x) => a + x, 0) / nums.length), 2), 0) / nums.length),
    };
  }, [filtrees]);

  const exportCSV = () => {
    const headers = ["Valeur", "Unité", "Heure", "Produit", "Fournisseur", "Lot interne", "Lot fournisseur", "DLC", "Statut", "Date arrivage"];
    ...
    link.href = url;
    link.download = `historique_${type}_${new Date().toISOString().slice(0,10)}.csv`;
    link.click();
  };
  ...
  return (
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 9999, background: "#f5f3ee", display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 16px", background: "#0a0a0a", borderBottom: "3px solid #c8a84b", flexShrink: 0 }}>
        ...
        <span style={{ color: "#c8a84b", fontWeight: 700, fontFamily: "'Syne', sans-serif", fontSize: 13 }}>{type === "poids" ? "⚖️ Suivi poids" : "🌡️ Suivi températures"}</span>
        <span style={{ fontSize: 11, color: "rgba(255,255,255,0.45)", fontFamily: "'Syne', sans-serif" }}>{filtrees.length} relevé{filtrees.length > 1 ? "s" : ""}</span>
      </div>

      <div style={{ padding: "12px 16px", background: "#fff", borderBottom: "1px solid #e8e0d0", flexShrink: 0, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        <div style={{ display: "flex", gap: 4, borderRadius: 9, border: "1.5px solid #e8e0d0", background: "#f9fafb", padding: 2 }}>
          {[{ k: "poids", l: "⚖️ Poids" }, { k: "temperature", l: "🌡️ Temp" }].map(b => (
            <button key={b.k} onClick={() => setType(b.k as any)} style={{ padding: "6px 12px", borderRadius: 8, background: type === b.k ? "#c8a84b" : "transparent", color: type === b.k ? "#0a0a0a" : "#6b7280", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 700, fontFamily: "'Syne', sans-serif" }}>{b.l}</button>
          ))}
        </div>
        <input value={q} onChange={e => setQ(e.target.value)} placeholder="🔍 Produit, fournisseur, lot…" style={{ ...champStyle, flex: 1, minWidth: 180 }} />
        <label style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 12, fontWeight: 700, color: "#6b7280" }}>Du <input type="date" value={du} onChange={e => setDu(e.target.value)} style={champStyle} /></label>
        <label style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 12, fontWeight: 700, color: "#6b7280" }}>Au <input type="date" value={au} onChange={e => setAu(e.target.value)} style={champStyle} /></label>
        <button onClick={exportCSV} style={{ padding: "8px 12px", borderRadius: 9, border: "1.5px solid #c8a84b", background: "#fffbf0", cursor: "pointer", fontSize: 12, fontWeight: 700, color: "#8a6f2e" }}>📥 Export CSV</button>
        {(q || du || au) && (<button onClick={() => { setQ(""); setDu(""); setAu(""); }} style={{ padding: "8px 12px", borderRadius: 9, border: "1.5px solid #e8e0d0", background: "#fff", cursor: "pointer", fontSize: 12, fontWeight: 700, color: "#6b7280" }}>✕ Réinit</button>)}
      </div>

      {stats && (<div style={{ padding: "10px 16px", background: "#fffbf0", borderBottom: "1px solid #e8e0d0", flexShrink: 0, display: "flex", gap: 20, flexWrap: "wrap", fontSize: 12, fontWeight: 700, color: "#8a6f2e" }}>
        <span>📊 Moyenne : <span style={{ fontSize: 14, color: "#1a2e1a" }}>{stats.moy.toFixed(1)} {unite}</span></span>
        <span>📉 Min : <span style={{ fontSize: 14, color: "#dc2626" }}>{stats.min.toFixed(1)} {unite}</span></span>
        <span>📈 Max : <span style={{ fontSize: 14, color: "#b45309" }}>{stats.max.toFixed(1)} {unite}</span></span>
        <span>σ Écart-type : <span style={{ fontSize: 14, color: "#6b7280" }}>{stats.stddev.toFixed(1)} {unite}</span></span>
        <span style={{ color: "#9ca3af" }}>({stats.n} mesure{stats.n > 1 ? "s" : ""})</span>
      </div>)}

      <div style={{ flex: 1, overflowY: "auto", padding: "12px 16px 40px" }}>
        {filtrees.length === 0 ? (
          <div style={{ textAlign: "center", padding: "3rem 1rem", background: "#fff", border: "1px solid #e8e0d0", borderRadius: 16 }}>
            <div style={{ fontSize: 36, marginBottom: 10 }}>{type === "poids" ? "⚖️" : "🌡️"}</div>
            <p style={{ margin: 0, fontWeight: 700, color: "#6b7280", fontFamily: "'Syne', sans-serif" }}>{lignes.length === 0 ? "Aucun relevé enregistré" : "Aucun relevé ne correspond aux filtres"}</p>
          </div>
        ) : (
          <div style={{ background: "#fff", border: "1px solid #e8e0d0", borderRadius: 14, overflow: "hidden" }}>
            <div style={{ display: "grid", gridTemplateColumns: "80px 1fr 120px 100px 80px", gap: 0, fontSize: 11, fontWeight: 700, color: "#6b7280", padding: "8px 14px", background: "#f9fafb", borderBottom: "1px solid #e8e0d0", textTransform: "uppercase", letterSpacing: "0.5px" }}>
              <div>Valeur</div><div>Produit · Fournisseur</div><div>Traçabilité</div><div>DLC / Statut</div><div>Heure</div>
            </div>
            {filtrees.map((l, i) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "80px 1fr 120px 100px 80px", gap: 0, padding: "10px 14px", borderBottom: i === filtrees.length - 1 ? "none" : "1px solid #f5f3ee", background: i % 2 === 0 ? "#fff" : "#fdfcfa", alignItems: "start" }}>
                <div style={{ fontSize: 16, fontWeight: 800, color: type === "poids" ? "#1a2e1a" : "#1d4ed8", fontFamily: "'Syne', sans-serif" }}>{l.valeur} {unite}</div>
                <div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#1a2e1a" }}>{l.a?.produit || "—"}</div>
                  <div style={{ fontSize: 11, color: "#9ca3af" }}>{l.a?.fournisseur || "—"}</div>
                  {l.a?.lot_interne && <div style={{ fontSize: 10, color: "#6b7280" }}>Lot interne : {l.a.lot_interne}</div>}
                </div>
                <div style={{ fontSize: 11, color: "#6b7280" }}>
                  {l.a?.lot_fournisseur ? <div style={{ fontWeight: 700, color: "#1a2e1a" }}>{l.a.lot_fournisseur}</div> : <div>—</div>}
                  {l.a?.date && <div style={{ color: "#9ca3af" }}>Arr. {l.a.date}</div>}
                </div>
                <div style={{ fontSize: 11, color: "#6b7280" }}>
                  <div>{l.a?.dlc ? `⏰ ${l.a.dlc}` : "—"}</div>
                  {l.a?.statut && <div style={{ color: l.a.statut === "validé" ? "#15803d" : l.a.statut === "refusé" ? "#dc2626" : l.a.statut === "sous réserve" ? "#b45309" : "#6b7280", fontWeight: 700, fontSize: 10, textTransform: "uppercase" }}>📋 {l.a.statut}</div>}
                  {l.a?.litige && <div style={{ color: "#dc2626", fontWeight: 700, fontSize: 10 }}>⚠️ Litige</div>}
                </div>
                <div style={{ fontSize: 11, color: "#6b7280", fontWeight: 600, textAlign: "right" }}>{formaterHeureMesure(l.at) || "—"}</div>
              </div>
            ))}
          </div>
        )}
