...
// l'impression du bon" vivaient dans ReconditionnementModule.tsx et migrent ici. On réutilise
// directement les types/fonctions déjà là-bas (génération PDF, libellés dépôt...) plutôt que de
// les dupliquer, pour ne jamais avoir deux versions qui divergent.
import {
  DEPOT_LABEL,
  nowFr,
  genererBonPdf,
  demandeTestPourImpression,
  envoyerBonReconditionnementPourImpressionPC,
} from "./ReconditionnementModule";
import type { Depot as DepotRecond, Demande as DemandeRecond, Transporteur as TransporteurRecond, Mouvement as MouvementRecond } from "./ReconditionnementModule";
...
export function PrestatairesModule({ onClose, userName, initialTab, canConfig = true }: { onClose: () => void; userName?: string; initialTab?: "dashboard" | "configuration"; canConfig?: boolean }) {
  ...
  // ── 07/09/2026 — Outils migrés depuis Reconditionnement → Configuration (voir import plus
  // haut) : Transporteurs (CRUD), Jeu de test pointage groupé NLT, Nettoyage des tests et du
  // journal des mouvements, Tester l'impression du bon. Écoutes Firebase dédiées (mêmes chemins
  // que ReconditionnementModule.tsx) — ce module a besoin de ses propres données à jour.
  const [transporteursRecond, setTransporteursRecond] = useState<TransporteurRecond[]>([]);
  ...
  const [demandesRecond, setDemandesRecond] = useState<DemandeRecond[]>([]);
  const [mouvementsRecond, setMouvementsRecond] = useState<MouvementRecond[]>([]);
  ...
  async function ajouterTransporteur() {
    if (!nvNom.trim()) { notifyRecond("error", "✗ Indique un nom"); return; }
    const champs = {
      nom: nvNom.trim(),
    ...
    if (transporteurEnEdition) {
      ...
      notifyRecond("success", "✓ Transporteur modifié");
    } else {
      await push(ref(db, "reconditionnement_transporteurs"), champs);
      notifyRecond("success", "✓ Transporteur ajouté");
    }
    setNvNom(""); setNvContact(""); setNvTelephone(""); setNvEmail(""); setTransporteurEnEdition(null);
  }

  function modifierTransporteur(t: TransporteurRecond) {
    setNvNom(t.nom); setNvContact(t.contact || ""); setNvTelephone(t.telephone || ""); setNvEmail(t.email || "");
  ...
  // ─── Jeu de test "Pointage groupé" (Configuration) — voir la version d'origine dans
  // ReconditionnementModule.tsx pour l'explication complète du garde-fou `test`. ───
  async function creerJeuDeTest() {
    ...
    try {
      ...
      notifyRecond("success", "🧪 Jeu de test créé — va dans Arrivage → « Pointer arrivage », bloc « NLT (reconditionnement) » du jour. Aucun stock réel touché.");
    } catch (err: any) {
      notifyRecond("error", `❌ Erreur : ${err.message}`);
    }
  }

  async function supprimerJeuDeTest() {
    const idsDemandes = demandesRecond.filter((d: any) => d.test).map(d => d.id);
    const idsArrivages = arrivagesLies.filter((a: any) => a.test).map((a: any) => a.id);
    if (!idsDemandes.length && !idsArrivages.length) { notifyRecond("error", "Aucune donnée de test à supprimer."); return; }
  ...
  async function supprimerDemandeTerminee(d: DemandeRecond) {
    try {
      const { get } = await import("firebase/database");
      const caissesEnvoyees = d.depot === "nlt" ? (d.caissesIfcoEnvoyees || 0) : 0;
      const caissesPleinesRecues = (d as any).retour?.caissesIfcoPleinesRecues || 0;
      ...
      const stockMvIdsASupprimer = mouvementsRecond.filter((m: any) => m.reconditionnement_demande_id === d.id).map(m => m.id);
      await Promise.all([
      ...
      notifyRecond("error", `❌ Erreur lors du nettoyage : ${err.message}`);
    }
  }
  ...
  // 14/09/2026 — Elinathan a essayé le correctif +/- (v du 14/09) puis a préféré revenir à la
  // valeur finale directe, MAIS avec la possibilité de la mettre en négatif (le stock réel côté
  // prestataire peut l'être) — donc plus de blocage sur v < 0 dans les 3 boutons plus bas.
  useEffect(() => {
          ...
          <div>
            ...
            <div id="stock-ifco-ajustement" style={{
              ...
              <div style={{ padding: "16px", background: COLORS.gray100, borderBottom: `1px solid ${COLORS.gray200}` }}>
                ...
                <p style={{ margin: "4px 0 0", fontSize: "12px", color: COLORS.gray600 }}>Corrige un stock affiché sur le Dashboard s'il ne correspond plus au stock réel. Tape directement la valeur que doit prendre le stock (peut être négative).</p>
              </div>
              <div style={{ padding: "16px", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "16px" }}>
                <div>
                  ...
                  <input type="number" value={ajustStockMoorea} onChange={(e) => setAjustStockMoorea(e.target.value)} placeholder="Nouvelle valeur (peut être négative)" style={{ width: "100%", padding: "8px 10px", border: `1px solid ${COLORS.gray200}`, borderRadius: "6px", fontSize: "13px", boxSizing: "border-box", marginBottom: "6px" }} />
                  ...
                  <button
                    onClick={async () => {
                      // 14/09/2026 — Retour à la valeur finale directe (pas un correctif +/-) :
                      // Elinathan tape le chiffre qu'elle veut voir comme stock, et ça peut être
                      // négatif (elle en a besoin pour refléter un stock réellement négatif côté
                      // prestataire). On ne bloque donc plus les valeurs négatives ici.
                      const v = parseInt(ajustStockMoorea);
                      if (!Number.isFinite(v)) { setNotification({ type: "error", message: "✗ Valeur invalide" }); return; }
                ...
                <div>
                  ...
                  <input type="number" value={ajustStockNlt} onChange={(e) => setAjustStockNlt(e.target.value)} placeholder="Nouvelle valeur (peut être négative)" style={{ width: "100%", padding: "8px 10px", border: `1px solid ${COLORS.gray200}`, borderRadius: "6px", fontSize: "13px", boxSizing: "border-box", marginBottom: "6px" }} />
                ...
                <div>
                  ...
                  <input type="number" value={ajustStockAndes} onChange={(e) => setAjustStockAndes(e.target.value)} placeholder="Nouvelle valeur (peut être négative)" style={{ width: "100%", padding: "8px 10px", border: `1px solid ${COLORS.gray200}`, borderRadius: "6px", fontSize: "13px", boxSizing: "border-box", marginBottom: "6px" }} />
          ...
          <div style={{ display: "grid", gap: "20px" }}>
            ...
            <div style={{ background: "white", borderRadius: "12px", overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.05)", border: `1px solid ${COLORS.gray200}` }}>
              <div style={{ padding: "16px", background: COLORS.gray100, borderBottom: `1px solid ${COLORS.gray200}` }}>
                ...
                <p style={{ margin: "4px 0 0", fontSize: "12px", color: COLORS.gray600 }}>Corrige un stock IFCO (Moorea/NLT) ou de carton Andès s'il ne correspond plus au stock réel. Tape directement la valeur que doit prendre le stock (peut être négative).</p>
              </div>
              <div style={{ padding: "16px", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "16px" }}>
                <div>
                  <div style={{ fontSize: "12px", fontWeight: "700", color: COLORS.gray600, marginBottom: "6px" }}>🏭 IFCO — Moorea (actuel : {formatCaisses(stockLevels.moorea)})</div>
            ...
            <div style={{ background: "white", borderRadius: "12px", overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.05)", border: `1px solid ${COLORS.gray200}` }}>
              <div style={{ padding: "16px", background: COLORS.gray100, borderBottom: `1px solid ${COLORS.gray200}` }}>
                <h3 style={{ margin: 0, fontSize: "15px", fontWeight: "700", color: COLORS.gray700 }}>🔗 Accès à l'espace prestataire</h3>
                <p style={{ margin: "4px 0 0", fontSize: "12px", color: COLORS.gray600 }}>Ouvre l'espace public d'un prestataire (celui qu'il reçoit par mail) directement depuis ici, pour vérifier ou faire une action à sa place.</p>
              </div>
              <div style={{ padding: "16px", display: "flex", gap: 10, flexWrap: "wrap" }}>
                <button onClick={() => window.open(`${window.location.origin}/?portail=nlt`, "_blank")} style={{ padding: "10px 16px", borderRadius: 8, border: "none", background: COLORS.secondary, color: "#fff", fontSize: 12.5, fontWeight: 700, cursor: "pointer" }}>
                  🔄 Espace NLT
                </button>
            ...
            <div style={{ background: "white", borderRadius: "12px", overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.05)", border: `1px solid ${COLORS.gray200}` }}>
              <div style={{ padding: "16px", background: COLORS.gray100, borderBottom: `1px solid ${COLORS.gray200}` }}>
                <h3 style={{ margin: 0, fontSize: "15px", fontWeight: "700", color: COLORS.gray700 }}>🧪 Tester l'impression du bon (reconditionnement)</h3>
                <p style={{ margin: "4px 0 0", fontSize: "12px", color: COLORS.gray600 }}>Envoie un bon factice (marqué "TEST") dans la file d'impression, sans créer de vraie demande — utile pour vérifier que le relais PC de l'entrepôt et l'imprimante A4 (Ricoh) fonctionnent bien, de bout en bout.</p>
              </div>
              <div style={{ padding: "16px" }}>
                <button
                  onClick={async () => {
                    try {
                      ...
                      notifyRecond("success", "🧪 Bon de test envoyé à l'impression — vérifie l'imprimante A4 à l'entrepôt");
                    } catch (err: any) {
                      notifyRecond("error", `❌ Erreur lors de l'envoi du test : ${err?.message || "erreur inconnue"}`);
                    }
                  }}
                  style={{ padding: "8px 16px", borderRadius: 8, border: "none", background: COLORS.tertiary, color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer" }}
                >
            ...
            <div style={{ background: "white", borderRadius: "12px", overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.05)", border: `1px solid ${COLORS.gray200}` }}>
              <div style={{ padding: "16px", background: COLORS.gray100, borderBottom: `1px solid ${COLORS.gray200}` }}>
                <h3 style={{ margin: 0, fontSize: "15px", fontWeight: "700", color: COLORS.gray700 }}>🚚 Transporteurs (reconditionnement)</h3>
              </div>
              <div style={{ padding: "16px" }}>
                    ...
                    transporteursRecond.map(t => (
                      <div key={t.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: `1px solid ${COLORS.gray100}`, background: transporteurEnEdition === t.id ? COLORS.primaryLight : "transparent" }}>
                        ...
                        <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                          <button onClick={() => modifierTransporteur(t)} style={{ padding: "6px 10px", borderRadius: 6, border: `1px solid ${COLORS.secondary}`, background: "#fff", color: COLORS.secondary, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                            ✏️ Modifier
                          </button>
            ...
            <div style={{ background: "white", borderRadius: "12px", overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.05)", border: `1px solid ${COLORS.gray200}` }}>
              <div style={{ padding: "16px" }}>
                ...
                <p style={{ margin: "0 0 12px", fontSize: 11.5, color: COLORS.gray600 }}>
                  Crée 4 fausses lignes de retour NLT (origines et lots différents, quantités variées) directement dans « Pointer arrivage », pour tester le pointage groupé. Aucun stock réel (IFCO, cartons) n'est touché.
                </p>
              ...
              <div style={{ padding: "16px" }}>
                ...
                {outilsTestVisibles && (
                  <div style={{ marginTop: 14 }}>
                    ...
                    {demandesTermineesRecond.length === 0 ? (
                      ...
                      <>
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            for (const d of demandesTermineesRecond.filter(d => demandesASupprimerTest.has(d.id))) {
                      ...
                      <>
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            notifyRecond("success", "🧹 Journal nettoyé — le stock actuel n'a pas été modifié");
                          }}
